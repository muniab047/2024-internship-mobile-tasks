...
class RemoteDataSourceImpl implements RemoteDataSource {
  ...
  Future<ProductModel> getProduct(GetProductParams getProductParams) async {
    ...
    final response = await client.get(
        Uri.parse(
            'https://g5-flutter-learning-path-be.onrender.com/api/v1/products/${getProductParams.id}'),
  ...
  Future<Unit> insertProduct(InsertProductParams insertProductParams) async {
    ...
    request.files.add(MultipartFile(
        'image',
        insertProductParams.image.openRead(),
        await insertProductParams.image.length(),
        filename: basename(insertProductParams.image.path),
  ...
  Future<Unit> deleteProduct(DeleteProductParams deleteProductParams) async {
    ...
    final response = await client.delete(
        Uri.parse(
            'https://g5-flutter-learning-path-be.onrender.com/api/v1/products/${deleteProductParams.id}'),
        headers: {'Content-Type': 'application/json'});
  ...
  Future<Unit> updateProduct(UpdateProductParams updateProductParams) async {
    ...
    final response = await client.put(
        Uri.parse(
            'https://g5-flutter-learning-path-be.onrender.com/api/v1/products/${updateProductParams.id}'),
        body: jsonString,
        headers: {'Content-Type': 'application/json'});
  ...
  Future<List<ProductModel>> getAllProducts() async {
    ...
    if (response.statusCode == 200) {
      ...
      final List<ProductModel> productModelList = productListJson
